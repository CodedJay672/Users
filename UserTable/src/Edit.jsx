...
const Edit = forwardRef(function Edit({ user }, ref ) {
  ...
  return (
    ...
    <dialog ref={ref} className="mx-auto my-auto bg-white rounded-lg shadow-lg px-4 sm:pd-8 pt-4 sm:pt-8 pb-0 m-4 max-w-lg w-5/6">
      <div className="flex items-center mb-4">
        <h2 className=" w-full text-center text-xl font-semibold text-secondary"><span className="font-bold">{user.name}</span></h2>
      </div>
      <form className="flex flex-col gap-4">
        ...
        <label htmlFor="company" className="text-sm text-secondary-50">Company Name</label>
        <input type="text" id="company" value={user.company.name} readOnly={true} className="border rounded-lg px-4 py-2 focus:outline-0" />
        ...
        <div className="flex justify-end gap-4 mt-8 mb-4">
          <button value="cancel" formMethod="dialog" className='px-4 py-2 rounded-lg border bg-primary color-tertiary-50'>Save</button>
